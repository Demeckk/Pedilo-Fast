	...
	**********************************/
	...
	function insert_stock($nombre, $un, $av) {
		...
		foreach ($sucursales as $elemento) {
			...
			$consulta = $conexion->prepare("INSERT INTO `stock` (`Cod_producto`, `Nombre`, `Cantidad`, `Cod_sucursal`, `unidad_medicion`, `aviso`) 
											VALUES (:cant, :nombre, '--', :suc, :un, :av)");
			$consulta->bindParam(':cant', $cant);
			$consulta->bindParam(':nombre', $nombre);
			$consulta->bindParam(':suc', $suc);
			$consulta->bindParam(':un', $un);
			$consulta->bindParam(':av', $av);
			$consulta->execute();
		}
	}
	...
	function desc_stock($cod, $cant, $suc) {
		require '../Modelos/conexion.php';
		$consulta = $conexion->prepare("UPDATE stock SET cantidad = cantidad - :cant WHERE Cod_producto = :cod AND Cod_sucursal = :sucursal");
		$consulta->bindParam(':cant', $cant);
		$consulta->bindParam(':cod', $cod);
